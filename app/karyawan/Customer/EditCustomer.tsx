...
const EditCustomer = (myprops: props) => {
    ...
    try {
      ...
      if (response.data.success !== true) {
        toast(response.data.message, {
          containerId: `EditCustomer-${myprops.item.id}`,
          type: "warning",
        });
      }
  ...
  return (
    <div>
      <ToastContainer containerId={`EditCustomer-${myprops.item.id}`} />
      <button
        type="button"
        onClick={() => openModal()}
        ...
        <form onSubmit={(e) => handleSubmit(e)} className="w-full">
          <div className="w-full p-6 border-b bg-gradient-to-r from-sky-600 to-sky-700">
            <div className="flex items-center gap-3">
              <div>
                ...
                <p className="text-sm text-sky-100">Pastikan data terisi dengan benar</p>
              </div>
            </div>
          </div>
          ...
          <div className="w-full p-6 space-y-4">
            <div className="space-y-2">
              <label htmlFor={`username-${myprops.item.id}`} className="block text-sm font-medium text-gray-700">
                Username
              </label>
            ...
            <div className="space-y-2">
              <label htmlFor={`nik-${myprops.item.id}`} className="block text-sm font-medium text-gray-700">
                NIK
              </label>
              <div className="relative rounded-lg border border-gray-300 shadow-sm focus-within:border-sky-500 focus-within:ring-1 focus-within:ring-sky-500">
                <input
            ...
            <div className="space-y-2">
              <label htmlFor={`name-${myprops.item.id}`} className="block text-sm font-medium text-gray-700">
                Nama Lengkap
              </label>
              <div className="relative rounded-lg border border-gray-300 shadow-sm focus-within:border-sky-500 focus-within:ring-1 focus-within:ring-sky-500">
                <input
            ...
            <div className="space-y-2">
              <label htmlFor={`address-${myprops.item.id}`} className="block text-sm font-medium text-gray-700">
                Alamat
              </label>
              <div className="relative rounded-lg border border-gray-300 shadow-sm focus-within:border-sky-500 focus-within:ring-1 focus-within:ring-sky-500">
                <textarea
            ...
            <div className="space-y-2">
              <label htmlFor={`phone-${myprops.item.id}`} className="block text-sm font-medium text-gray-700">
                Nomor Telepon
              </label>
              <div className="relative rounded-lg border border-gray-300 shadow-sm focus-within:border-sky-500 focus-within:ring-1 focus-within:ring-sky-500">
                <input
